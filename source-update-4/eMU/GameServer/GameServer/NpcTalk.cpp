...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (gCustomNpcCollector.NpcTalk(lpNpc, lpObj) != 0)
	{
		return 1;
	}
	...
	gNpcTalkMessagek.NpcTalk(lpNpc,lpObj);
	#endif
...
void CNpcTalk::NpcElpis(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	pMsg.level[0] = lpObj->ChaosSuccessRate;
...
void CNpcTalk::NpcOsbourne(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	pMsg.level[0] = gServerInfo.m_JewelOfHarmonyItemSmeltMixRate1[lpObj->AccountLevel];

	pMsg.level[1] = gServerInfo.m_JewelOfHarmonyItemSmeltMixRate2[lpObj->AccountLevel];
