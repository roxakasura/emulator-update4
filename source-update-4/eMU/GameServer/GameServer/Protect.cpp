...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if(memcmp(gServerInfo.m_CustomerName, this->m_AuthInfo.CustomerName,sizeof(this->m_AuthInfo.CustomerName)) != 0)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
	...
	if (this->m_AuthInfo.Status == AUTH_SERVER_STATUS_USER_NOT_FOUND)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
