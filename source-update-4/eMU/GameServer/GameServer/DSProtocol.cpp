...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg) // OK
{
	...
	if (gServerInfo.m_DLCreateSwitch == 1) //criação do DL
	{
	...
	#if(GAMESERVER_DOWNGRADE<=101)
	if (gServerInfo.m_DLCreateSwitch == 1)
	{
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	pMsg.ViewNextExperience = lpObj->Experience;
	#endif
...
void GDCreateItemSend(int aIndex,BYTE map,BYTE x,BYTE y,int index,BYTE level,BYTE dur,BYTE Option1,BYTE Option2,BYTE Option3,int LootIndex,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus,DWORD duration) // OK
{
	...
	if (gServerInfo.m_AncientItemTapeSwitch == 2)
	{
		pMsg.SetOption = 0;
	}
	else
...
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, char * LogName) // OK
{
	...
	{
		lpObj->Coin1 += value1;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(25), value1);
	}
	if (value2 > 0)
	{
		lpObj->Coin2 += value2;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(26), value2);
	}
	if (value3 > 0)
	{
		lpObj->Coin3 += value3;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(27), value3);
	}

	if (value1 < 0)
	{
		lpObj->Coin1 += value1;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(28), value1);
	}
	if (value2 < 0)
	{
		lpObj->Coin2 += value2;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(29), value2);
	}
	if (value3 < 0)
	{
		lpObj->Coin3 += value3;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(30), value3);
	}

	LogAdd(LOG_BLUE, "[%s][%s] WZ_SetCoin(%s) Val1: %d, Val2: %d, Val3: %d", lpObj->Account, lpObj->Name, LogName, value1, value2, value3);
}