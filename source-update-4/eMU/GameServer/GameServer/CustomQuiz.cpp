...
void CCustomQuiz::ProcState_EMPTY(CUSTOM_QUIZ_INFO* lpInfo) // OK
{
	if(lpInfo->RemainTime > 0 && lpInfo->RemainTime <= (lpInfo->RuleInfo.AlarmTime*60))
	{
		if((lpInfo->AlarmMinSave=(((lpInfo->RemainTime%60)==0)?((lpInfo->RemainTime/60)-1):(lpInfo->RemainTime/60))) != lpInfo->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(492),lpInfo->RuleInfo.Name,(lpInfo->AlarmMinLeft+1));
		}
	}
	...
	if(lpInfo->RemainTime <= 0)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(493),lpInfo->RuleInfo.Name);
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(494),lpInfo->RuleInfo.Name,CustomQuiz.Question);

			LogAdd(LOG_GREEN,"[CustomQuiz] Start");
		...
		{
			LogAdd(LOG_RED,"[CustomQuiz] Erro");
			this->Active		=  0;
			this->IndexSelected = -1;
			this->IndexInfo		= -1;
		}
...
void CCustomQuiz::ProcState_START(CUSTOM_QUIZ_INFO* lpInfo) // OK
{
	if(this->Active == 0)
	{
		LogAdd(LOG_GREEN,"[CustomQuiz] Finish");
	...
	if(lpInfo->RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(496),lpInfo->RuleInfo.Name);
...
{
#if GAMESERVER_CLIENTE_UPDATE >= 0
	...
	if (this->GetInfo(this->IndexSelected,&CustomQuiz) == 1)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(495),this->m_CustomQuizInfo[this->IndexInfo].RuleInfo.Name,lpObj->Name);
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(498),this->m_CustomQuizInfo[this->IndexInfo].RuleInfo.Name,CustomQuiz.Answer);

			LogAdd(LOG_GREEN,"[CustomQuiz] Winner: %s",lpObj->Name);
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(497),this->m_CustomQuizInfo[this->IndexInfo].RuleInfo.Name);
		}
	}
...
void CCustomQuiz::StartQuiz()
{
	...
	LogAdd(LOG_RED,"[Set Quiz Start] At %2d:%2d:00",hour,minute);
