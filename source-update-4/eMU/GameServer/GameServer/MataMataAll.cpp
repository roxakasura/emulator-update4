// CMataMataAll.cpp: interface for the CMataMataAll class.
//
//////////////////////////////////////////////////////////////////////
...
CMataMataAll gMataMataAll;

CMataMataAll::CMataMataAll() // OK
{
	this->m_RemainTime = 0;
	...
	{ 
		...
		if(this->m_RemainTime <= (this->m_TotalTime-gServerInfo.m_EventKillAllTimeToEnter) && this->m_EventKillAllEnter == 1)
		{
			if (this->m_Members < 2)
			{
				...
				LogAdd(LOG_BLUE,"[EVENT KILL ALL] Canceled");
		...
		if(this->m_RemainTime <= 0 && (this->m_EventKillAllEnter != 0 || this->m_EventKillAllActive != 0))
		{
				...
				LogAdd(LOG_BLUE,"[EVENT KILL ALL] Time is over");
		...
		if(this->m_RemainTime <= 2 && this->m_EventKillAllActive != 0)
		{
			...
			for(int n=0;n < MAX_KILLALL_USER;n++)
			{
				if(OBJECT_RANGE(this->User[n].Index) != 0)
				{
					if(this->User[n].Rank == 1)
					{
						...
						LogAdd(LOG_BLUE,"[EVENT PVP] (Winner1: %s)",gObj[this->User[n].Index].Name);
					}
					if(this->User[n].Rank == 2)
					{
						...
						LogAdd(LOG_BLUE,"[EVENT PVP] (Winner2: %s)",gObj[this->User[n].Index].Name);
					}
					if(this->User[n].Rank == 3)
					{
						...
						LogAdd(LOG_BLUE,"[EVENT PVP] (Winner3: %s)",gObj[this->User[n].Index].Name);
					}
			...
			LogAdd(LOG_BLUE,"[EVENT PVP] Finish");
			this->Clear();
...
bool CMataMataAll::CommandEventKillAll(LPOBJ lpObj,char* arg) // OK 
{
	...
	LogAdd(LOG_BLUE,"[EVENT KILL ALL] Start by %s",lpObj->Name);

	return 1;
}

void CMataMataAll::CommandEventKillAllJoin(LPOBJ lpObj,char* arg) // OK 
{
	if(gServerInfo.m_EventKillAllSwitch == 0)
	{
		return;
	...
	LogAdd(LOG_BLUE,"[EVENT KILL ALL] Member add (%s)",lpObj->Name);
}

bool CMataMataAll::AddUser(int aIndex) // OK
{
...
void CMataMataAll::CalcUserRank() // OK
{
	for(int n=0;n < MAX_KILLALL_USER;n++)
	{
		if(OBJECT_RANGE(this->User[n].Index) == 0)
...
bool CMataMataAll::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
...
void CMataMataAll::CheckUser() // OK
{
	for(int n=0;n < MAX_KILLALL_USER;n++)
	{
		if(OBJECT_RANGE(this->User[n].Index) == 0)
...
void CMataMataAll::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	if(gServerInfo.m_EventKillAllSwitch == 0)
	{
		return;
...
bool CMataMataAll::Respawn(LPOBJ lpObj,int* gate,int* map,int* x,int* y,int* dir,int* level) // OK
{
	if(gServerInfo.m_EventKillAllSwitch == 0)
	{
		return 0;
