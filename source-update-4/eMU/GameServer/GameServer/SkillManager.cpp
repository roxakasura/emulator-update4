...
bool CSkillManager::CheckSkillTarget(LPOBJ lpObj,int aIndex,int bIndex,int type) // OK
{
	...
	if (lpObj->KillAll != 0 && gObj[SummonIndex].KillAll != 0 && gMataMataAll.CheckPlayerTarget(lpObj, &gObj[SummonIndex]) != 0)
	{
		return 1;
	}
...
bool CSkillManager::SkillManaShield(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	this->GCSkillAttackSend(lpObj,lpSkill->m_index,lpTarget->Index,lpSkill->m_index);
	return 1;
}
