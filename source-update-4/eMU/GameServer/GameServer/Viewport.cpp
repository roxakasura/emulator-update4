...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.skillstate = lpTarget->m_ViewSkillState;
		...
		info.skillstate = gEffectManager.GenerateEffectList(lpTarget,&send[size],&InfoSize);
...
void CViewport::GCViewportMonsterSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.state = gEffectManager.GenerateEffectList(lpTarget,&send[size],&InfoSize);
...
void CViewport::GCViewportSummonSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.state = lpTarget->m_ViewSkillState;
...
void CViewport::GCViewportChangeSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.skin[0] = SET_NUMBERHB((lpTarget->Change & 0xFFFF) & 0xFFFF);
		info.skin[1] = SET_NUMBERLB((lpTarget->Change & 0xFFFF) & 0xFFFF);
...
void CViewport::GCViewportCastleSiegeWeaponSend(int aIndex,int tx,int ty) // OK
{
	...
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		info.dwViewSkillState = lpTarget->m_ViewSkillState;
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	info.skillstate = lpObj->m_ViewSkillState;
...
void CViewport::GCViewportSimpleMonsterSend(LPOBJ lpObj) // OK
{
	...
	info.state = lpObj->m_ViewSkillState;
...
void CViewport::GCViewportSimpleChangeSend(LPOBJ lpObj) // OK
{
	...
	info.skin[0] = SET_NUMBERHB((lpObj->Change & 0xFFFF) & 0xFFFF);
	info.skin[1] = SET_NUMBERLB((lpObj->Change & 0xFFFF) & 0xFFFF);
