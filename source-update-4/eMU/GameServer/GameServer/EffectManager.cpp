...
int CEffectManager::GenerateEffectList(LPOBJ lpObj,BYTE* lpMsg,int* size) // OK
{
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		if(lpObj->Effect[n].IsEffect() != 0)
		{
			if(lpObj->Effect[n].m_index == EFFECT_GAME_MASTER && (KALIMA_MAP_RANGE(lpObj->Map) != 0 || DS_MAP_RANGE(lpObj->Map) != 0 || BC_MAP_RANGE(lpObj->Map) != 0 || CC_MAP_RANGE(lpObj->Map) != 0 || IT_MAP_RANGE(lpObj->Map) != 0 || DA_MAP_RANGE(lpObj->Map) != 0 || DG_MAP_RANGE(lpObj->Map) != 0 || IG_MAP_RANGE(lpObj->Map) != 0))
			{
				continue;
			}
...
void CEffectManager::GCEffectStateSend(LPOBJ lpObj,BYTE state,int effect) // OK
{
...
void CEffectManager::GCPeriodicEffectSend(LPOBJ lpObj,WORD group,WORD value,BYTE state,DWORD time,int effect) // OK
{
	#if((GAMESERVER_DOWNGRADE==201 && GAMESERVER_LANGUAGE==5) || GAMESERVER_DOWNGRADE==301)
