...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(damage > 0)
	{
		...
		gObjectManager.CharacterLifeCheck(lpObj, lpTarget, (damage - ShieldDamage), 0, flag, effect, ((lpSkill == 0) ? 0 : lpSkill->m_index), ShieldDamage, combo);
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (lpObj->KillAll == 1 && lpTarget->KillAll == 1 && gMataMataAll.CheckPlayerTarget(lpObj, lpTarget) == 0)
	{
		return 0;
	}
