...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gMataMataAll.UserDieProc(lpObj, lpTarget);
		return;
	}
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if(BC_MAP_RANGE(map) != 0 || CC_MAP_RANGE(map) != 0 || DS_MAP_RANGE(map) != 0 || IT_MAP_RANGE(map) != 0)
	{
...
void gObjExtItemApply(LPOBJ lpObj)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		{
			if (lpObj->Inventory[n].m_Durability != 0.0f)
			{
				if (lpObj->Inventory[n].m_IsValidItem == true)
				{
					lpObj->ExcellentDamage += lpObj->Inventory[n].IsExtExcellentDamage();
				}
			}
		}
	}
