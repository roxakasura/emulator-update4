...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	if (lpObj->KillAll != 0)
	{
		result = gMataMataAll.Respawn(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
...
void CObjectManager::CharacterCalcSD(LPOBJ lpObj) // OK
{
	...
	#if(ALGOQUEVOUUSARUMDIA == 1)
	lpObj->MaxShield = ((((value*gServerInfo.m_ShieldGaugeConstA)/10)+(((lpObj->Level)*(lpObj->Level))/gServerInfo.m_ShieldGaugeConstB))+(lpObj->Defense/2))/gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].ShieldDivider;
	#elif(GAMESERVER_DOWNGRADE>101)
	lpObj->MaxShield = (((value*gServerInfo.m_ShieldGaugeConstA)/10)+(((lpObj->Level)*(lpObj->Level))/gServerInfo.m_ShieldGaugeConstB))+(lpObj->Defense/2);
	#else
	lpObj->MaxShield = 0;
	#endif
}
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	while(change != 0)
	{
		...
		gItemNewOption.CalcItemOptionCombination(lpObj,1);
#endif
		#if(GAMESERVER_UPDATE>=402)
...
#if(GAMESERVER_CLIENTE_UPDATE > 3)
	gItemNewOption.CalcItemOptionCombination(lpObj,0);
#endif
	#if(GAMESERVER_UPDATE>=402)
	...
	//Sistema HP bonus
	if(gServerInfo.m_HpBonus[lpObj->AccountLevel] == 1 && gServerInfo.m_HpBonusMax > 0)
	{
		lpObj->AddLife += lpObj->MasterReset*gServerInfo.m_HpBonusMax;
	}
	//FIm
