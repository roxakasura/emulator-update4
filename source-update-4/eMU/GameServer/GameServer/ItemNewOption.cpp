...
CItemOptionCombination gItemNewOption;
//////////////////////////////////////////////////////////////////////
...
void CItemOptionCombination::InsertOption(LPOBJ lpObj, int index, int value, bool flag) // OK
{
	if (flag == 0)
	{
		if (index == NEW_ITEM_OPTION_ADD_STRENGTH || index == NEW_ITEM_OPTION_ADD_DEXTERITY || index == NEW_ITEM_OPTION_ADD_VITALITY || index == NEW_ITEM_OPTION_ADD_ENERGY || index == NEW_ITEM_OPTION_ADD_LEADERSHIP)
		{
			return;
		}
	}
	...
	switch (index)
	{
	case NEW_ITEM_OPTION_ADD_STRENGTH:
		lpObj->AddStrength += value;
		break;
	case NEW_ITEM_OPTION_ADD_DEXTERITY:
		lpObj->AddDexterity += value;
		break;
	case NEW_ITEM_OPTION_ADD_VITALITY:
		lpObj->AddVitality += value;
		break;
	case NEW_ITEM_OPTION_ADD_ENERGY:
		lpObj->AddEnergy += value;
		break;
	case NEW_ITEM_OPTION_ADD_LEADERSHIP:
		lpObj->AddLeadership += value;
		break;
	case NEW_ITEM_OPTION_ADD_MIN_PHYSI_DAMAGE:
		lpObj->PhysiDamageMinLeft += value;
		lpObj->PhysiDamageMinRight += value;
		break;
	case NEW_ITEM_OPTION_ADD_MAX_PHYSI_DAMAGE:
		lpObj->PhysiDamageMaxLeft += value;
		lpObj->PhysiDamageMaxRight += value;
		break;
	case NEW_ITEM_OPTION_MUL_MAGIC_DAMAGE:
		lpObj->MagicDamageMin += (lpObj->MagicDamageMin * value) / 100;
		lpObj->MagicDamageMax += (lpObj->MagicDamageMax * value) / 100;
		break;
	case NEW_ITEM_OPTION_ADD_PHYSI_DAMAGE:
		lpObj->PhysiDamageMinLeft += value;
		lpObj->PhysiDamageMinRight += value;
		lpObj->PhysiDamageMaxLeft += value;
		lpObj->PhysiDamageMaxRight += value;
		break;
	case NEW_ITEM_OPTION_MUL_ATTACK_SUCCESS_RATE:
		lpObj->AttackSuccessRate += (lpObj->AttackSuccessRate * value) / 100;
		break;
	case NEW_ITEM_OPTION_ADD_DEFENSE:
		lpObj->Defense += value;
		break;
	case NEW_ITEM_OPTION_ADD_MAX_LIFE:
		lpObj->AddLife += value;
		break;
	case NEW_ITEM_OPTION_ADD_MAX_MANA:
		lpObj->AddMana += value;
		break;
	case NEW_ITEM_OPTION_ADD_MAX_BP:
		lpObj->AddBP += value;
		break;
	case NEW_ITEM_OPTION_ADD_BP_RECOVERY:
		lpObj->BPRecovery += value;
		break;
	case NEW_ITEM_OPTION_ADD_CRITICAL_DAMAGE_RATE:
		lpObj->CriticalDamageRate += value;
		break;
	case NEW_ITEM_OPTION_ADD_CRITICAL_DAMAGE:
		lpObj->CriticalDamage += value;
		break;
	case NEW_ITEM_OPTION_ADD_EXCELLENT_DAMAGE_RATE:
		lpObj->ExcellentDamageRate += value;
		break;
	case NEW_ITEM_OPTION_ADD_EXCELLENT_DAMAGE:
		lpObj->ExcellentDamage += value;
		break;
	case NEW_ITEM_OPTION_ADD_SKILL_DAMAGE:
		lpObj->SkillDamageBonus += value;
		break;
	case NEW_ITEM_OPTION_ADD_DOUBLE_DAMAGE_RATE:
		lpObj->DoubleDamageRate += value;
		break;
	case NEW_ITEM_OPTION_ADD_IGNORE_DEFENSE_RATE:
		lpObj->IgnoreDefenseRate += value;
		break;
	case NEW_ITEM_OPTION_MUL_SHIELD_DEFENSE:
		if (lpObj->Inventory[1].m_Index >= GET_ITEM(6, 0) && lpObj->Inventory[1].m_Index < GET_ITEM(7, 0))
		{
			lpObj->Defense += (lpObj->Defense * value) / 100;
		}
		break;
	case NEW_ITEM_OPTION_MUL_TWO_HAND_PHYSI_DAMAGE:
		...
		break;
	case NEW_ITEM_OPTION_ADD_PHYSI_DAMAGE_BY_STRENGTH:
		...
		break;
	case NEW_ITEM_OPTION_ADD_PHYSI_DAMAGE_BY_DEXTERITY:
		...
		break;
	case NEW_ITEM_OPTION_ADD_DEFENSE_BY_DEXTERITY:
		lpObj->Defense += ((value == 0) ? 0 : (lpObj->Dexterity / value));
		break;
	case NEW_ITEM_OPTION_ADD_DEFENSE_BY_VITALITY:
		lpObj->Defense += ((value == 0) ? 0 : (lpObj->Vitality / value));
		break;
	case NEW_ITEM_OPTION_ADD_MAGIC_DAMAGE_BY_ENERGY:
		lpObj->MagicDamageMin += ((value == 0) ? 0 : (lpObj->Energy / value));
		lpObj->MagicDamageMax += ((value == 0) ? 0 : (lpObj->Energy / value));
		break;
	case NEW_ITEM_OPTION_ADD_ICE_DAMAGE:
		lpObj->AddResistance[0] += value;
		break;
	case NEW_ITEM_OPTION_ADD_POISON_DAMAGE:
		lpObj->AddResistance[1] += value;
		break;
	case NEW_ITEM_OPTION_ADD_LIGHTNING_DAMAGE:
		lpObj->AddResistance[2] += value;
		break;
	case NEW_ITEM_OPTION_ADD_FIRE_DAMAGE:
		lpObj->AddResistance[3] += value;
		break;
	case NEW_ITEM_OPTION_ADD_EARTH_DAMAGE:
		lpObj->AddResistance[4] += value;
		break;
	case NEW_ITEM_OPTION_ADD_WIND_DAMAGE:
		lpObj->AddResistance[5] += value;
		break;
	case NEW_ITEM_OPTION_ADD_WATER_DAMAGE:
