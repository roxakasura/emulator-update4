...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
		...
		case 0xAA:
			...
			break;
		case 0xAB:
			gDuel.CGDuelEndRecv((PMSG_DUEL_END_RECV*)lpMsg,aIndex);
			break;
		case 0xAC:
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->message[0] == '/')
	{
		if (gCommandManager.ManagementCore(&gObj[aIndex], lpMsg->message) == 1)
		{
			return;
		}
		else
...
void CGCharacterCreateRecv(PMSG_CHARACTER_CREATE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpMsg->Class == DB_CLASS_MG && lpObj->ClassCode < 3)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}

	if (lpMsg->Class == DB_CLASS_DL && lpObj->ClassCode < 4)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
...
void gObjSetSkillEffect(LPOBJ lpObj, BYTE state, int effect)
{
	if (state == 1)
	{
		lpObj->m_ViewSkillState |= effect;
	}
	else if (state == 0)
	{
		lpObj->m_ViewSkillState &= ~effect;
	...
	GCSkillEffectSend(lpObj->Index, state, effect);
}

void GCSkillEffectSend(int aIndex, BYTE state, int  effect)
{
