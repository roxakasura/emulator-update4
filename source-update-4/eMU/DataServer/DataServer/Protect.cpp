...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if(this->m_AuthInfo.PackageType < 0)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
		this->InvalidDataServerUpdateMessageError();
		return;
	}
...
inline void CProtect::InvalidDataServerUpdateMessageError() // OK
{
	#if(PROTECT_STATE==1)
	...
	wsprintf(buff,"Você está usando o dataserver de um update incompatível\ncom a sua licença.\nEntre em contato com o suporte.");
