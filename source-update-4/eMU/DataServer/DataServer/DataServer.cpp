...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	GetPrivateProfileString("DataServerInfo","Token","",Token,sizeof(Token),".\\DataServer.ini");
	...
	GetPrivateProfileString("DataServerInfo","CustomerHardwareId","",CustomerHardwareId,sizeof(CustomerHardwareId),".\\DataServer.ini");
		...
		gProtect.StartAuth(AUTH_SERVER_TYPE_S3_DATA_SERVER);
	#elif(DATASERVER_DOWNGRADE==101)
		gProtect.StartAuth(AUTH_SERVER_TYPE_S0_DATA_SERVER);
	#endif

	char buff[256];

	wsprintf(buff,"[%s] RoxGaming DataServer (QueueSize : %d)",DATASERVER_VERSION,0);
