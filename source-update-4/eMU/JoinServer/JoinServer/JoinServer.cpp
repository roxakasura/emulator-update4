...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	GetPrivateProfileString("JoinServerInfo","Token","",Token,sizeof(Token),".\\JoinServer.ini");
	...
	#if(JOINSERVER_DOWNGRADE==101)
	gProtect.StartAuth(AUTH_SERVER_TYPE_S0_JOIN_SERVER);
	#elif(JOINSERVER_DOWNGRADE==301)
	gProtect.StartAuth(AUTH_SERVER_TYPE_S3_JOIN_SERVER);
	#else
	...
	wsprintf(buff,"[%s] RoxGaming JoinServer (QueueSize : %d) (AccountCount : %d/%d)",JOINSERVER_VERSION,0,0,0);
