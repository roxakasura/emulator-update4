...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if(this->m_AuthInfo.PackageType < JOINSERVER_CLIENTE_UPDATE)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
		this->InvalidJoinServerUpdateMessageError();
		return;
	}
...
inline void CProtect::InvalidJoinServerUpdateMessageError() // OK
{
	#if(PROTECT_STATE==1)
