...
bool CProtect::AuthServerDataSend(eAuthServerType ServerType) // OK
{
	...
	memcpy(pMsg.Token,Token,sizeof(pMsg.Token));
...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if(this->m_AuthInfo.PackageType < CONNECTSERVER_CLIENTE_UPDATE)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
		this->InvalidConnectServerUpdateMessageError();
		return;
	}
...
inline void CProtect::InvalidConnectServerUpdateMessageError() // OK
{
	#if(PROTECT_STATE==1)
	...
	wsprintf(buff,"Você está usando o joinserver de um update incompatível\ncom a sua licença.\nEntre em contato com o suporte.");
