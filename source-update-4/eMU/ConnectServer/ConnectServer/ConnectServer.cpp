...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	GetPrivateProfileString("ConnectServerInfo","Token","",Token,sizeof(Token),".\\ConnectServer.ini");
	...
	#if(CONNECTSERVER_DOWNGRADE==101)
	gProtect.StartAuth(AUTH_SERVER_TYPE_S0_CONNECT_SERVER);
	#elif(CONNECTSERVER_DOWNGRADE==301)
	gProtect.StartAuth(AUTH_SERVER_TYPE_S3_CONNECT_SERVER);
	#else
	...
	wsprintf(buff,"[%s] RoxGaming ConnectServer (QueueSize : %d)",CONNECTSERVER_VERSION,0);
