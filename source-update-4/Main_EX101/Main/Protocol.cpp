...
BOOL ProtocolCoreEx(BYTE head,BYTE* lpMsg,int size,int key) // OK
{
	switch(head)
	{
		...
		case 0xFB:
			...
			{
				case 0x17:
					#if(MAIN_CLIENT_UPDATE > 4)
					GCRunAndCatchRecv((PMSG_PEGAPEGA_RECV*)lpMsg);
					#endif
					break;
			}
			break;
...
void GCCharacterInfoRecv(PMSG_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	//*pMainSpeedFrameTwo = 0.001500000f;

	switch (gUser.m_Player.Class)
	{
	case 0:
		SetByte(0x00510E56, ((gProtect.m_MainInfo.DWMaxAttackSpeed >= 0xFFFF) ? 0x02 : 0x0F));
		if (gProtect.m_MainInfo.DWMaxAttackSpeed >= 0xFFFF)
		{
			*pMainSpeedFrameOne = 0.001500000f;
		...
		break;
	case 1:
		SetByte(0x00510E56, ((gProtect.m_MainInfo.DKMaxAttackSpeed >= 0xFFFF) ? 0x02 : 0x0F));
		if (gProtect.m_MainInfo.DWMaxAttackSpeed >= 0xFFFF)
		{
			*pMainSpeedFrameOne = 0.001500000f;
			*pMainSpeedFrameTwo = 0.0008100f;
		...
		break;
	case 2:
		SetByte(0x00510E56, ((gProtect.m_MainInfo.FEMaxAttackSpeed >= 0xFFFF) ? 0x02 : 0x0F));
		if (gProtect.m_MainInfo.DWMaxAttackSpeed >= 0xFFFF)
		{
		...
		break;
	case 3:
		SetByte(0x00510E56, ((gProtect.m_MainInfo.MGMaxAttackSpeed >= 0xFFFF) ? 0x02 : 0x0F));
		if (gProtect.m_MainInfo.DWMaxAttackSpeed >= 0xFFFF)
		{
			*pMainSpeedFrameOne = 0.001500000f;
			*pMainSpeedFrameTwo = 0.0017500f; //BK e MG funcionaram 100%
		}
		break;
	case 4:
		SetByte(0x00510E56, ((gProtect.m_MainInfo.DLMaxAttackSpeed >= 0xFFFF) ? 0x02 : 0x0F));
		break;
	}
}
...
int ViewEventPegaPega = 0;

void GCRunAndCatchRecv(PMSG_PEGAPEGA_RECV* lpMsg) // OK
{
	if(lpMsg->Active == 1)
	{
		SetByte(0x0054CFBA + 2, 0); //AUTOCTRL ATIVO
		SetByte(0x0054D108 + 2, 0); //AUTOCTRL ATIVO
		SetByte(0x0054B5B0, 0xC3); //REMOVE NAME
	}
	else
	{
		SetByte(0x0054CFBA + 2, 0x80); //AUTOCTRL ATIVO
		SetByte(0x0054D108 + 2, 0x80); //AUTOCTRL ATIVO
		SetByte(0x0054B5B0, 0x55); //REMOVE NAME
	}
}
	...
	if (gPacketManager.AddData(lpMsg, size) != 0 && gPacketManager.ExtractPacket(EncBuff) != 0)
	{
		...
		if (EncBuff[0] == 0xC3 || EncBuff[0] == 0xC4)
		{
			if (EncBuff[0] == 0xC3)
			{
				...
				((void(__thiscall*)(void*, int))0x00404870)((void*)0x07AFEA08, MAIN_PACKET_SERIAL);

				EncBuff[1] = (*(BYTE*)(MAIN_PACKET_SERIAL))++;

				((void(__thiscall*)(void*, int))0x004049E0)((void*)0x07AFEA08, MAIN_PACKET_SERIAL);
